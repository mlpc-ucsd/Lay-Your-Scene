...
from layout_evaluation import LayoutPlot, evaluate_counting, LayoutType
...
def evaluate_nsr_counting(
    ...
    data_iters = [x["iter"] for x in input_file]
    data_captions = [x["prompt"] for x in input_file]

    # extract labels
    data_labels = extract_concepts_from_prompts(data_captions)
    ...
    output_json = []
    for id, iter, caption, layout in zip(
        data_ids, data_iters, data_captions, layouts
    ):
        # plot layouts
    ...
    result_dict = evaluate_counting(output_file_path, val_file)
    return result_dict


def run_nsr_counting_evaluation(
    config: Config,
    model: Any,
    ...
    for cfg_scale in cfg_scales:
        ...
        result_dict = evaluate_nsr_counting(
            config,
            sample_fn="sample_with_cfg",
            diffusion=diffusion,
            model=model,
...
def main():
    ...
    parser.add_argument("--eval-dir", type=str, default="evaluation_output/counting")

    # arguments to control the layout
    parser.add_argument(
        "--input-file",
        type=str,
        default="layout_evaluation/nsr/counting.val.json",
    ...
    # run evaluation
    results = run_nsr_counting_evaluation(
        config=config,
        model=model,
        preprocessor=preprocessor,
        diffusion=diffusion,
